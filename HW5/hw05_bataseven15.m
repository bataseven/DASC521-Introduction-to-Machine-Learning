...
plot(decision_x, decision_y ,'-k','LineWidth',1);
title(strcat("P = ",num2str(Pruning_Parameter)));
fprintf('RMSE is %g when P is %g\n', calculate_RMSE([decision_x decision_y], training_set), Pruning_Parameter);
...
for i = 1 : N_Parent - 1
    ...
    average_left = mean(y(index_left));
    average_right = mean(y(index_right));
